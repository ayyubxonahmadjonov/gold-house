...
class UserAgrrementsDartBloc
    extends Bloc<UserAgrrementsDartEvent, UserAgrrementsDartState> {
  UserAgrrementsDartBloc() : super(UserAgrrementsDartInitial()) {
    on<GetUserAgreementsDataEvent>(getUserAgreements);
  }
  Future<void> getUserAgreements(
    GetUserAgreementsDataEvent event,
    Emitter<UserAgrrementsDartState> emit,
  ) async {
    ...
    if (result.isSuccess) {
      final userAgreements =
          (result.result as List).map((e) => Agreement.fromJson(e)).toList();
